...
class ItemsetData:
   ...
   def writeFullStats(outFile):
	   ...
	   for entity in self.counter:
		  outputFile.write(''.join([str(i) + ' ' for i in sorted(entity, key=int)]) + "->" + str(self.counter[entity]) +"\n")  
	   ...
	   for pattern in sorted(unintended, key=unintended.get, reverse=True):
		  if unintended[pattern] > 1:
			 outputFile.write(''.join([str(i) + ' ' for i in sorted(pattern, key=int)]) + "->" + str(unintended[pattern]) +"\n")
