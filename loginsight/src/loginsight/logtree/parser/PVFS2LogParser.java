...
public class PVFS2LogParser implements LogTreeParser {
	...
	private static void test2() {
		try {
			String filename = "pvfs2-server.log";
			PVFS2LogParser parser = new PVFS2LogParser();
			List<LogElement> logElems = parser.parse(new BufferedReader(
					new FileReader(filename)), 0, -1);
			for (int i=0; i<logElems.size(); i++) {
				logElems.get(i).print(System.out);
			}
		}catch(Exception e) {
			e.printStackTrace();
		}
	...
	private static void test1() {
		try {
			PVFS2LogParser parser = new PVFS2LogParser();
			//String message = "[P 04/27 05:04] Start times (hr:min:sec):  05:04:05.584  05:04:04.580  05:04:03.572  05:04:02.568  05:04:01.563  05:04:00.559";
			//String message = "[P 04/27 05:03] request scheduler       :             0             0             0             0             0             0";
			//String message = "[D 04/27 05:04] [SM Exiting]: (0x952aeb8) io_sm:start_flow (error code: 0), (action: DEFERRED)";
			String message = "[D 04/27 05:04] ATTRIB: retrieved attributes from DISK for key 9223372036854775263 \nuid = 0, mode = 0, type = 2";
			LogElement e = parser.parseMessage(message);
			e.updateDepth(0);
			e.print(System.out);
		}catch(Exception e) {
			e.printStackTrace();
