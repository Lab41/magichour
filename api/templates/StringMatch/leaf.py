...
class Leaf(object):
    def __init__(self, log_line, index=None, index_val=None):
        ...
        self.template_line = log_line.msg
        self.template_line_split = log_line.msg.split()
        self.index = index
    ...
    def print_groups(self, index, include_text=False):
        for line in self.log_lines:
            try:
                if include_text:
                    print '%s,%d,%s'%(mktime(line.ts.timetuple()), index, line.msg)
                else:
    ...
    def split_leaf(self, min_items_for_split, skip_count, min_word_pos_entropy, min_percent, verbose=False):
        ...
        line_length = max([len(self.log_lines[i].msg.split()) for i in range(len(self.log_lines))])
        # Build position dependent word counter
        word_counts = [defaultdict(int) for i in range(line_length)]

        # Count words in each line
        for line in self.log_lines:
            for i, word in enumerate(line.msg.split()[skip_count:]):
        ...
        leafs = {}
        for line in self.log_lines:
            line_split = line.msg.split()[skip_count:]
            try:
