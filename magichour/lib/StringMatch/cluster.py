...
class Cluster(object):
    ...
    def check_for_match(
        ...
        '''
        # If our template string matches then check to see if any of the leafs
        # match
        if cosine_sim(
            ...
            if self.index is not None and self.index_val is not None:
                if len(line_split) > skip_count + \
                        self.index and self.index_val != line_split[skip_count + self.index]:
                    if verbose:
    ...
    def add_to_leaf(self, line, threshold, skip_count):
        ...
        for i in range(len(self.leafs)):
            if self.leafs[i].check_for_match(
        ...
        if len(leaf_indexes_that_match) == 1:
            ...
            if self.leafs[leaf_indexes_that_match[0]].index_val is not None:
                self.leafs[
                    leaf_indexes_that_match[0]].add_to_leaf(
                    line, threshold, skip_count)
            else:
                self.leafs[
                    leaf_indexes_that_match[1]].add_to_leaf(
                    line, threshold, skip_count)
        else:
            ...
            raise ValueError(
                "Received %d matches, 1 or 2 expected" %
                len(leaf_indexes_that_match))
    ...
    def split_leaf(
        ...
        for i, leaf in enumerate(self.leafs):
            if leaf.get_num_lines() > min_items_for_split:
                replacement_leafs = leaf.split_leaf(
                    min_items_for_split, skip_count, min_word_pos_entropy, min_percent)
                if replacement_leafs is not None:  # If every item is the same
                    replacements.append((i, replacement_leafs))

        # IMPORTANT: Reverse replacements so that deletions don't change
        # indexes
