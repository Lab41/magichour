# Purpose - Score and plot algorithm results against generated frequent
# itemset data
...
algorithmScore = namedtuple('algorithmScore',
                            ['totalScore',
                             'numEvents',
                             'proposedEvents',
                             'exactMatches',
...
def scoreResults(answerKey, algorithmResults, parameter):
    ...
    for algorithmAnswer in algorithmResults:
        ...
        for correctAnswer in answerKey:
            totalDifference = len(
                correctAnswer.symmetric_difference(
                    algorithmAnswer.template_ids))
            totalSimilarity = len(
                correctAnswer.intersection(
                    algorithmAnswer.template_ids))
            onesidedDifference = len(
    ...
    totalScore = float(exactMatches + (.5 * strongMatches) +
                       (.01 * weakMatches) - (.01 * noMatches)) / numAnswers
...
def run_score_paris(inputData):
    ...
    if verbose:
        if (highFreq):
            filename = "paris" + str(tau) + "hifreq.results"
        else:
            filename = "paris" + str(tau) + ".results"
...
def run_score_fpgrowth(inputData):
    ...
    sys.stderr.write(
        "Running FPGrowth with minimum support " +
        str(support) +
        "\n")
    fpgrowthResults = events.fp_growth(testData.transactionsList, support)
    if verbose:
        if (highFreq):
            filename = "fpgrowth" + str(support) + "hifreq.results"
        else:
            filename = "fpgrowth" + str(support) + ".results"
...
def run_score_glove(inputData):
    ...
    gloveResults = events.glove(
        testData.transactionsList,
        num_components,
        glove_window,
        epochs)
    if verbose:
        if (highFreq):
            filename = "glove" + str(epochs) + "hifreq.results"
        else:
            filename = "glove" + str(epochs) + ".results"
...
def main(argv):
    ...
    # Main variables to tweak for generating real events,transactions,events
    # and plots
    ...
    # Initiating ItemSet object and generating real events for seeding in
    # transactions
    testData = ItemsetData(
        minRealTemplateID,
        maxRealTemplateID,
        minFillTemplateID,
        maxFillTemplateID)
    testData.createRealEvents(
        numRealEvents,
        minRealEventLength,
        maxRealEventLength)
    sys.stderr.write("\nCreated " +
                     str(len(testData.realEventsList)) +
                     " Real Events")

    # Generating transactions
    testData.createTransactions(
        numTransactions,
        transactionLength,
        probabilityEvent,
        paddingLimit)
    sys.stderr.write("\nCreated " +
                     str(len(testData.transactionsList)) +
                     " transactions\n")

    # Show histogram of template usage
    if (showPlots):
        testData.plotTemplateUsage("Standard Data")
    ...
    # Show histogram of template usage
    if (showPlots):
        testDataFreqs.plotTemplateUsage("High Frequency Data")
    ...
    if testGlove:
        ...
        for step in epochs:
            ...
            test_cases_freq.append(
                (testDataFreqs, 16, 10, step, True, verbose))
