...
public class SpectralClusterer {
  ...
  SymmetricMatrix _simMatrix = null;
  
  List _insts = null;
  
  SimilarityFunction _simFunc = null;
  ...
  public SpectralClusterer(List insts, SimilarityFunction simFunc) {
	  _insts = insts;
	  _simFunc = simFunc;
  }
  ...
  public void build(int[] instCluLabels) throws Exception {
	  if (_simMatrix == null) {
		  precomputeSimMatrix();
	  }
	  
	  buildClusterer(_simMatrix);
	  for (int i=0; i<cluster.length; i++) {
		  instCluLabels[i] = cluster[i];
	  }
  }
...
//   */
//  public int clusterInstance(int index) throws java.lang.Exception {
...
//  }
   ...
   */
  public void buildClusterer(SymmetricMatrix simMatrix) throws java.lang.Exception {
	_simMatrix = simMatrix;
    int n = _insts.size();
    DoubleMatrix2D w;
    if(useSparseMatrix)
    ...
    for(int i = 0; i < n; i++)
      for(int j = i; j < n; j++) {
        double sim = simMatrix.get(i, j);
        if((r == -1) || (sim > r)) {
          sim = Math.exp( (sim * sim) / (2 * sigma_sq));
          w.set(i, j, sim);
          w.set(j, i, sim);
        }
     }
